 ...
 * pending. NOTE: keep in lockstep with queries.ts.
 */
export function backfillClaimedChests(): void {
  resync();
  if (db.app_meta[CHEST_BACKFILL_KEY] === '1') return;
  const gs = db.game_state;
  let claimed: number[];
  try { claimed = JSON.parse(gs.claimed_chests || '[]'); } catch { claimed = []; }
  const next = backfilledClaims(gs.longest_streak, claimed);
  if (next.length !== claimed.length) gs.claimed_chests = JSON.stringify(next);
  db.app_meta[CHEST_BACKFILL_KEY] = '1';
  persist();
}
